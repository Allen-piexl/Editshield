...
paths = ['./instruct-pix2pix-main/002_Data/EOT-G/Gen', 
         './instruct-pix2pix-main/002_Data/EOT-G/Adv', 
         './instruct-pix2pix-main/002_Data/EOT-G/AdvGen', 
         './instruct-pix2pix-main/002_Data/EOT-G/Ori', 
         './instruct-pix2pix-main/002_Data/EOT-G/EXCEL']
...
for i in range(len(train_data_path)):
    ...
    for j in range(len(image_list)):
        if image_list[j].endswith('_0.jpg'):
            ...
            images = pipe(prompt, image=Image.open(input_path).resize((512, 512)), num_inference_steps=100, image_guidance_scale=1.2,
            ...
            for step in range(max_steps):
                ...
                loss_smooth = F.mse_loss(apply_gaussian_smoothing(perturbed_images, kernel_size, sigma), gaussian_data)
                ...
                if step % 10 == 0:
                    print(f"PGD loss - step {step}, total loss: {total_loss.item()}, mse loss: {loss_mse.item()}, smooth loss: {loss_smooth.item()}, perceptual loss: {loss_perceptual.item()}")
            ...
            generator = torch.Generator("cuda").manual_seed(33)
            images = pipe(prompt, image=Image.fromarray(np.uint8(noised_imgs.transpose(1, 2, 0) * 255)), num_inference_steps=100,
            ...
            image_features_benign = clip_similarity.encode_image(torch.tensor(x.transpose(2, 0, 1)).unsqueeze(0).to(device))
            image_features_gen = clip_similarity.encode_image(torch.tensor(x_gen.transpose(2, 0, 1)).unsqueeze(0).to(device))
            image_feature_adv = clip_similarity.encode_image(torch.tensor(x_adv.transpose(2, 0, 1)).unsqueeze(0).to(device))
            image_features_attack = clip_similarity.encode_image(torch.tensor(x_gen_attack.transpose(2, 0, 1)).unsqueeze(0).to(device))
